...
class hjUDPproxy {
    public static void main(String[] args) throws Exception {
        ...
        InputStream SRTSPConf = new FileInputStream("config.crypto");
        if (SRTSPConf == null) {
            System.err.println("config.crypto file not found!");
            System.exit(1);
        }

        Properties SRTSPProperties = new Properties();
        SRTSPProperties.load(SRTSPConf);
        getConfigs srtspConf = new getConfigs(SRTSPProperties);
	    ...
	    mySimplifiedSRTSPDatagramSockets inSocket = new mySimplifiedSRTSPDatagramSockets(inSocketAddress, srtspConf);
        DatagramSocket outSocket = new DatagramSocket();
        byte[] buffer = new byte[4 * 1024];
       
        while (true) {
          DatagramPacket inPacket = new DatagramPacket(buffer, buffer.length);
 	      inSocket.mySSPReceive(inPacket);  // if remote is unicast
